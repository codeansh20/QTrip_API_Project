...
public class test_case_04 {
    ...
    public void Testcase04() {
        ...
        Assert.assertEquals(registerStatusCode1, 201,
                "Failed to verify successful new registration for first time");
        String payload1 = resPayload;
        JsonPath jsonPath1 = new JsonPath(payload1);
        Assert.assertTrue(jsonPath1.getBoolean("success"),
                "Failed to verify success field is true for first time new registration");


        int registerStatusCode2 = registerAPI_Call();
        String payload2 = resPayload;
        JsonPath jsonPath2 = new JsonPath(payload2);
        Assert.assertEquals(registerStatusCode2, 400,
                "Failed to verify unsuccessful duplicate registration");
        Assert.assertFalse(jsonPath2.getBoolean("success"),
                "Failed to validate success field is false");
        String message = jsonPath2.getString("message");
        Assert.assertEquals(message, "Email already exists",
                "Failed to validate the message as: Email already exists");
